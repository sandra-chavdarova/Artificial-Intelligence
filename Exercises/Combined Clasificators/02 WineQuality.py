...
Последната колона ја содржи оценката за квалитетот на виното која може да има вредности помеѓу 0 и 10.
Сите колони содржат податоци од непрекинат тип.

Ваша задача е да истренирате класификатор кој ќе предвидува дали некое вино има добар или лош квалитет.
Поделете ги оценките од последната колона во две класи: 1 - добар квалитет и 0 - лош квалитет на следниот начин:
...
Првите X проценти од податочното множество се користат за тестирање, а останатите за тренирање.
Делот од множеството што треба да се користи за тестирање се чита од стандардниот влез.

Истренирајте дрво на одлука (criterion='gini') и најдете го атрибутот со најмала важност.
Овој атрибут отстранете го од податочното множество.

Потребно е да истренирајте модел на невронска мрежа со 15 неврони во скриениот слој,
активациска функција (activation) ReLU, рата на учење (learning_rate_init) 0.001 и 200 епохи (max_iter).
Притоа користете го множеството од кое е отстранета колоната на атрибутот со најмала важност.
Ова множество дополнително скалирајте го со StandardScaler и MinMaxScaler.

Потребно е да ги пресметате и испечатите точноста на класификаторот трениран со податоците скалирани
со StandardScaler и на класификаторот трениран со податоците скалирани со MinMaxScaler.
...
def main(dataset):
    ...
    x = int(input())
    threshold = int(x / 100 * len(dataset))
    ...
    feature_importances = [float(x) for x in classifier_tree.feature_importances_]
    least_important = feature_importances.index(min(feature_importances))

    train_x_2 = list()
    for t in train_x:
        row = [t[i] for i in range(len(t)) if i != least_important]
    ...
    classifier_neural1 = MLPClassifier(15, activation="relu", learning_rate_init=0.001, max_iter=200, random_state=0)
    standard_scaler = StandardScaler()
    standard_scaler.fit(train_x_2)
    classifier_neural1.fit(standard_scaler.transform(train_x_2), train_y)
    predictions_standard = classifier_neural1.predict(standard_scaler.transform(test_x_2))
    accuracy_standard = accuracy_score(test_y, predictions_standard)
    print(f"Tocnost so StandardScaler: {accuracy_standard}")

    classifier_neural2 = MLPClassifier(15, activation="relu", learning_rate_init=0.001, max_iter=200, random_state=0)
    min_max_scaler = MinMaxScaler()
    min_max_scaler.fit(train_x_2)
    classifier_neural2.fit(min_max_scaler.transform(train_x_2), train_y)
    predictions_minmax = classifier_neural2.predict(min_max_scaler.transform(test_x_2))
    accuracy_minmax = accuracy_score(test_y, predictions_minmax)
    print(f"Tocnost so MinMaxScaler: {accuracy_minmax}")
