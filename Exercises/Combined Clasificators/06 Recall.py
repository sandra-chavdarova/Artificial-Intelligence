...
Сите атрибути кои ги содржи се од нумерички тип.
...
Изградете ги моделите на класификација и одредете кој од нив има најголема точност.
На стандарден излез испечатете кој е класификаторот со најголема точност.
(Najgolema tocnost ima klasifikatorot Naive Bayes/Decision Tree/Random Forest/MLP)
...
На пример, ако класификаторот со најголема точност и уште еден класификатор ја предвидат класата 0, а останатите два класификатори ја предвидат класата 1,
тогаш класата 0 ќе има 3 гласа, а класата 1 ќе има 2 гласа. Класификаторот ја предвидува класата 0.
...
def main(dataset):
    ...
    threshold_0 = int(X / 100 * len(dataset_0))
    threshold_1 = int(X / 100 * len(dataset_1))
    train_set = dataset_0[:threshold_0] + dataset_1[:threshold_1]
    test_set = dataset_0[threshold_0:] + dataset_1[threshold_1:]
    ...
    classifiers.append(("Decision Tree", DecisionTreeClassifier(criterion="entropy", random_state=0)))
    classifiers.append(("Random Forest", RandomForestClassifier(n_estimators=4, criterion="entropy", random_state=0)))
    classifiers.append(
        ("MLP", MLPClassifier(hidden_layer_sizes=10, activation="relu", learning_rate_init=0.001, random_state=0)))
    ...
    for (label, classifier) in classifiers:
        ...
        if accuracy > max_accuracy:
            ...
            max_label = label
    print(f"Najgolema tocnost ima klasifikatorot {max_label}")
