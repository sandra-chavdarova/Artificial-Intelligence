"""
Потребно е да направите класификација на тип на вино со колекција од класификатори на следниот начин.
Податочното множество е предефинирано во променливата dataset.
Секој ред се однесува на еден од трите типови вино означени со 0, 1, или 2.
За секој тип има 13 карактеристики (реални вредности) кои се однесуваат на резултати од хемиска анализа.
Последната колона претставува класа.

Од стандарден влез се читаат 2 вредности x1 и x2 кои претставуваат процент за поделба на множеството.
Вредностите на x1 и x2 се во рангот (0, 1), и притоа за сите тест примери важи дека x2 > x1.
...
За тестирање се користат записите од податочното множество во рангот од x2% до 100% од секоја класа.
Потребно е да ја пресметате и испечатите точноста на колекцијата од класификатори.
Еден запис се смета за точно класифициран доколку класата во која припаѓа е точно предвидена од барем 2 класификатори.
...
def main(dataset):
    ...
    train_set_1 = class0[:int(p1 * len(class0))] + class1[:int(p1 * len(class1))] + class2[:int(p1 * len(class2))]
    train_set_2 = class0[int(p1 * len(class0)):int(p2 * len(class0))] + \
                  class1[int(p1 * len(class1)):int(p2 * len(class1))] + \
                  class2[int(p1 * len(class2)):int(p2 * len(class2))]
    train_set_3 = class0[:int(p2 * len(class0))] + class1[:int(p2 * len(class1))] + class2[:int(p2 * len(class2))]
    ...
    test_set = class0[int(p2 * len(class0)):] + class1[int(p2 * len(class1)):] + class2[int(p2 * len(class2)):]
    ...
    for i in range(len(predictions1)):
        local = 0
        if predictions1[i] == test_y[i]:
            local += 1
        if predictions2[i] == test_y[i]:
            local += 1
        if predictions3[i] == test_y[i]:
            local += 1
...
dataset = [[14.23, 1.71, 2.43, 15.6, 127, 2.8, 3.06, .28, 2.29, 5.64, 1.04, 3.92, 1065, 0],
