...
Топката не може да се влече, туку само да се турка.
...
а позицијата на човечето и топката се менуваат и се читаат од стандарден влез.
...
class Football(Problem):
    ...
    def h(self, node):
        ...
        first = goal[0]
        second = goal[1]
        return min(abs(ball[0] - first[0]) + abs(ball[1] - first[1]),
                   abs(ball[0] - second[0]) + abs(ball[1] - second[1])) / 2
