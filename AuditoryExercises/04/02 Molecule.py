...
На табла 7x9 поставени се три атоми (внимавајте, двата H-атоми се различни: едниот има линк во десно,
а другиот има линк во лево). Полињата обоени во сива боја претставуваат препреки.
...
со кој ќе ја вратите секвенцата на движења која треба да се направи
за да може атомите од почетната позиција да се доведат до бараната позиција.
...
class Molecule(Problem):
    ...
    def h(self, node):
        ...
        if h1[1] != o[1]:  # different rows
            if h1[0] != o[0] - 1:  # H1 not right next to O
                value += 2
            else:
                value += 1
        else:  # same row
            if h1[0] > o[0]:  # H1 is on the right of O (instead of left)
                value += 3
            elif h1[0] < o[0] - 1:  # H1 is on the left, but not immediately
                value += 1

        if h2[1] != o[1]:  # different rows
            if h2[0] != o[0] + 1:  # H1 not right next to O
                value += 2
            else:
                value += 1
        else:  # same row
            if h1[0] < o[0]:  # H2 is on the left of O (instead of right)
                value += 3
            elif h1[0] > o[0] + 1:  # H2 is on the right, but not immediately
                value += 1

        if h1[1] == h2[1] and h1[1] != o[1]:  # H1 and H1 in the same row, O in different
            value -= 1
